...
# in this version the FOMC info only contians members who give a speech before cast the vote
# we need to use speech data from socre_speeches.py
...
# regions = pd.get_dummies(regions, columns = ['Region', 'Division']).reset_index(drop=True)

#############################################################################
# meeting info
meeting_data = pd.read_feather('parsed_labeled_dataset_augment.ftr')
meeting_data = meeting_data.loc[meeting_data['source']=='Original']
...
# here we only keep the meeting and members who give a speech before the meeting 
meeting_data['date'] = pd.to_datetime(meeting_data['meeting'].str[4:12])
meeting_data['year'] = meeting_data['date'].dt.year

# speecch data is compiled in 16_score_speeches.py
meeting_info = meeting_data[['meeting', 'date']].copy()
meeting_info = meeting_info.drop_duplicates()
meeting_info = meeting_info.sort_values(by=['date']).reset_index(drop=True)

speech_data = pd.read_feather('speech_data/parsed_speech.ftr')
# only keep last name
...
speech_data = speech_data[['date', 'speaker']].copy()
...
speech_data = speech_data[['related_meeting', 'speaker']].copy()
speech_data = speech_data.drop_duplicates()
speech_data.columns = ['meeting', 'name']

# now we only wanna keep the person who gave a speech and also vote in the following meeting
meeting_data = pd.merge(speech_data, meeting_data, on=['meeting', 'name'],  how='inner', sort=False)
...
reg_data.to_feather('fomc_info_reg_data_only_contain_member_give_speech.ftr')
